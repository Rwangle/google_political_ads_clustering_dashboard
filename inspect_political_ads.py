...
scrpt_dir = os.path.dirname(os.path.abspath(
    __file__))
DB_PATHH = os.path.join(  scrpt_dir,"db","political_ads.db")

con = sqlite3.connect(DB_PATHH)
...
subsection("Clustering columns")
for col in [
    "Ad_Type","Age_Targeting","Gender_Targeting",
    "Geo_Targeting_Included","Geo_Targeting_Excluded",
    "Impressions","Num_of_Days",
...
# california deep dive (reopening connection bc we closed it above..
# should restructure this but whatever)
con = sqlite3.connect(DB_PATHH)
